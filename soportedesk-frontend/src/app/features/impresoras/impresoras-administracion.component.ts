...
@Component({
    ...
    imports: [ModalComponent, ImpresoraFichaComponent, ImpresoraFormComponent, ImpresorasListViewComponent],
    template: `
    ...
    <app-modal title="Ficha técnica" [open]="viewing !== null" size="wide" (closed)="closeView()">
      @if (viewing) {
        <app-impresora-ficha [impresora]="viewing" (editRequested)="onEdit($event)" />
      }
    </app-modal>
