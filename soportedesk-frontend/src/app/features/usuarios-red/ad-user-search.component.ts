...
@Component({
  ...
  template: `
      ...
      <form class="ad-query-bar" (ngSubmit)="search()">
        <label class="ad-query-input">
          ...
          <input
            ...
            autofocus
          />
          <button type="button" class="icon-button clear-query" *ngIf="hasAnyFilter" (click)="clearFilters()" aria-label="Limpiar busqueda">
      ...
      <div class="catalog-filters">
        <label class="field catalog-field">
          ...
          <select name="ouSelect" [(ngModel)]="ou" (ngModelChange)="queueSearch()">
            ...
            <option *ngFor="let option of organizationalUnits" [value]="option.value">
        ...
        <label class="field catalog-field">
          ...
          <select name="officeSelect" [(ngModel)]="oficina" (ngModelChange)="queueSearch()">
            ...
            <option *ngFor="let option of offices" [value]="option.value">
              {{ option.value }} ({{ option.total }})
            </option>
          </select>
        </label>
    ...
    <section class="ad-result-list" *ngIf="searched && results.length">
      <article
        class="ad-result-card"
        *ngFor="let user of results; trackBy: trackBySam"
        tabindex="0"
        role="button"
      ...
      >
        ...
        <div class="result-identity">
          ...
          <a *ngIf="user.mail" [href]="'mailto:' + user.mail" (click)="$event.stopPropagation()">{{ user.mail }}</a>
        </div>
