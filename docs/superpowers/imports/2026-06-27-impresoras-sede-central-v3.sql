...
-- Version 3: catalogos por staging, joins insensibles a tildes/mayusculas, dependencias/subdependencias amarradas a sede, sin GO.
...
12 MESES', NULL, NULL, NULL, N'Activa');
    ...
    -- Diagnostico de normalizacion: estas salidas deben salir vacias o mostrar valores ya equivalentes por tilde/mayuscula.
    SELECT DISTINCT i.sede AS sede_import, s.nombre AS sede_catalogo
    FROM #impresoras_import i
    LEFT JOIN dbo.sedes s ON s.nombre COLLATE Latin1_General_CI_AI = i.sede COLLATE Latin1_General_CI_AI
    WHERE s.id IS NULL;

    SELECT DISTINCT i.sede, i.dependencia AS dependencia_import, d.nombre AS dependencia_catalogo
    FROM #impresoras_import i
    LEFT JOIN dbo.sedes s ON s.nombre COLLATE Latin1_General_CI_AI = i.sede COLLATE Latin1_General_CI_AI
    LEFT JOIN dbo.dependencias d ON d.nombre COLLATE Latin1_General_CI_AI = i.dependencia COLLATE Latin1_General_CI_AI AND d.sede_id = s.id
    WHERE i.dependencia IS NOT NULL AND d.id IS NULL;

    SELECT DISTINCT i.dependencia, i.subdependencia AS subdependencia_import, sd.nombre AS subdependencia_catalogo
    FROM #impresoras_import i
    LEFT JOIN dbo.sedes s ON s.nombre COLLATE Latin1_General_CI_AI = i.sede COLLATE Latin1_General_CI_AI
    LEFT JOIN dbo.dependencias d ON d.nombre COLLATE Latin1_General_CI_AI = i.dependencia COLLATE Latin1_General_CI_AI AND d.sede_id = s.id
    LEFT JOIN dbo.subdependencias sd ON sd.nombre COLLATE Latin1_General_CI_AI = i.subdependencia COLLATE Latin1_General_CI_AI AND sd.dependencia_id = d.id
    WHERE i.subdependencia IS NOT NULL AND sd.id IS NULL;

    -- Catalogos que alimentan los combos del sistema
    INSERT INTO dbo.sedes (nombre)
    SELECT DISTINCT i.sede
    FROM #impresoras_import i
    WHERE NOT EXISTS (SELECT 1 FROM dbo.sedes s WHERE s.nombre COLLATE Latin1_General_CI_AI = i.sede COLLATE Latin1_General_CI_AI);

    -- Reparacion de nombres antiguos si una carga previa creo dependencias con abreviaturas.
    DECLARE @sedeCentralId BIGINT;
    SELECT @sedeCentralId = id FROM dbo.sedes WHERE nombre COLLATE Latin1_General_CI_AI = N'Sede Central' COLLATE Latin1_General_CI_AI;

    DECLARE @oldDepId BIGINT;
    DECLARE @newDepId BIGINT;

    SELECT @oldDepId = id FROM dbo.dependencias WHERE sede_id = @sedeCentralId AND nombre COLLATE Latin1_General_CI_AI = N'Organo de Control Institucional' COLLATE Latin1_General_CI_AI;
    SELECT @newDepId = id FROM dbo.dependencias WHERE sede_id = @sedeCentralId AND nombre COLLATE Latin1_General_CI_AI = N'Oficina de Control Institucional' COLLATE Latin1_General_CI_AI;
    ...
    SET @oldDepId = NULL;
    SET @newDepId = NULL;
    SELECT @oldDepId = id FROM dbo.dependencias WHERE sede_id = @sedeCentralId AND nombre COLLATE Latin1_General_CI_AI = N'PRED' COLLATE Latin1_General_CI_AI;
    SELECT @newDepId = id FROM dbo.dependencias WHERE sede_id = @sedeCentralId AND nombre COLLATE Latin1_General_CI_AI = N'Direccion Ejecutiva PRED' COLLATE Latin1_General_CI_AI;
    IF @oldDepId IS NOT NULL AND @newDepId IS NULL
    ...
    WHERE i.tipo_impresora IS NOT NULL
      AND NOT EXISTS (SELECT 1 FROM dbo.tipos_impresora ti WHERE ti.nombre COLLATE Latin1_General_CI_AI = i.tipo_impresora COLLATE Latin1_General_CI_AI);

    INSERT INTO dbo.dependencias (nombre, sede_id)
    SELECT DISTINCT i.dependencia, s.id
    FROM #impresoras_import i
    JOIN dbo.sedes s ON s.nombre COLLATE Latin1_General_CI_AI = i.sede COLLATE Latin1_General_CI_AI
    WHERE i.dependencia IS NOT NULL
      AND NOT EXISTS (
          SELECT 1 FROM dbo.dependencias d
          WHERE d.nombre COLLATE Latin1_General_CI_AI = i.dependencia COLLATE Latin1_General_CI_AI AND d.sede_id = s.id
      );
    ...
    FROM #impresoras_import i
    JOIN dbo.sedes s ON s.nombre COLLATE Latin1_General_CI_AI = i.sede COLLATE Latin1_General_CI_AI
    JOIN dbo.dependencias d ON d.nombre COLLATE Latin1_General_CI_AI = i.dependencia COLLATE Latin1_General_CI_AI AND d.sede_id = s.id
    WHERE i.subdependencia IS NOT NULL
      AND NOT EXISTS (
          SELECT 1 FROM dbo.subdependencias sd
          WHERE sd.nombre COLLATE Latin1_General_CI_AI = i.subdependencia COLLATE Latin1_General_CI_AI AND sd.dependencia_id = d.id
      );
    ...
    IF EXISTS (
        ...
        LEFT JOIN dbo.dependencias d ON d.nombre COLLATE Latin1_General_CI_AI = i.dependencia COLLATE Latin1_General_CI_AI AND d.sede_id = s.id
        LEFT JOIN dbo.subdependencias sd ON sd.nombre COLLATE Latin1_General_CI_AI = i.subdependencia COLLATE Latin1_General_CI_AI AND sd.dependencia_id = d.id
        LEFT JOIN dbo.tipos_impresora ti ON ti.nombre COLLATE Latin1_General_CI_AI = i.tipo_impresora COLLATE Latin1_General_CI_AI
        WHERE s.id IS NULL
